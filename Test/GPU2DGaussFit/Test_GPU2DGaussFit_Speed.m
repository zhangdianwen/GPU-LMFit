...
%       To study the computation speed of two-dimenstional Gaussian fit using GPU_LMFit 
%
...
ImgDimArr = 7:1:30; % 21 is the maximum by Smith

for jj = 1:length(ImgDimArr)
    
    ImgDim = ImgDimArr(jj);       %linear size of fit region in pixels.
    sigmax = sigma0*ImgDim;
    sigmay = sigmax; % sigmax == sigmay => 5 fitting paras <=> SmithMLEFitType = 4 
    n = 5;
    ...
    NFactor = (sigmax/1.0*sigmay/1.0);
    NPhotons = NPhotons0*NFactor;
    ...
    ImgData = NPhotons/(2*pi*sigmax*sigmay)*exp(-1/2*((xi-x0)/sigmax).^2)...
    ...
    tic
    [FittedX InfoNum] = GPU2DGaussFit_RS(single(reshape(permute(ImgData,[2,1,3]),1,ImgDim*ImgDim*NumOfFits)), ...
    ...
    tic
    [FittedX InfoNum] = GPU2DGaussFit_RD(reshape(permute(ImgData,[2,1,3]),1,ImgDim*ImgDim*NumOfFits), ...
        ImgDim, init_ins, 0, 0, 0, GPU_Block_Size, GPU_Grid_Size);
    ...
    if(ImgDim<=21 && NoSmithMLE==0)
        fprintf('\n\n');
        
        % Parameters for Smith MLE programs
        SmithGPUMLE_Iters = 20;
        SmithGPUMLE_FitType = 4;    %   1 - position,bg,N only;
        %   2 - also PSF sigma;
        %   3 - z position;
...
Result2File = [ImgDimArr' Smith_MLE_Speed' GPU_LMFit_MLE_RS_Speed' GPU_LMFit_MLE_RD_Speed'];
outputfilename = [mfilename('fullpath') '_Speed@' TimeStr '.txt'];
save(outputfilename, 'Result2File', '-ASCII');

%%Display Figures
figure('Name', 'Accuracy')
subplot(1,2,1)
plot(ImgDimArr, SmithMLE_stdx0, 'ro', ...
    ImgDimArr, GPU_LMFit_MLE_RS_stdx0, 'bx', ...
    ImgDimArr, GPU_LMFit_MLE_RD_stdx0, 'g+');
axis tight
xlabel('Dimenstion D')
ylabel('Precision - std')
title('Experimental-x0');
subplot(1,2,2)
plot(ImgDimArr, SmithMLE_stdy0, 'ro', ...
    ImgDimArr, GPU_LMFit_MLE_RS_stdy0, 'bx', ...
    ImgDimArr, GPU_LMFit_MLE_RD_stdy0, 'g+');
axis tight
title('Experimental-y0');
legend('SmithMLE','GPU\_LMFit\_RS\_MLE','GPU\_LMFit\_RD\_MLE');
xlabel('Dimenstion D')
ylabel('Precision - std')

figure('Name', 'SmithMLE and GPU2DGassFit speed comparison')
plot(ImgDimArr, Smith_MLE_Speed, 'ro-', ...
    ImgDimArr, GPU_LMFit_MLE_RS_Speed, 'bx-', ...
    ImgDimArr, GPU_LMFit_MLE_RD_Speed, 'g+-')
axis tight
xlabel('Dimenstion D')
ylabel('Speed (fits//second)')
legend('SmithMLE','GPU\_LMFit\_RS\_MLE','GPU\_LMFit\_RD\_MLE');

%% Display comparison
figure('Name', 'GPU_LMFit_MLE_Speed./Smith_MLE_Speed')
plot(ImgDimArr,GPU_LMFit_MLE_RS_Speed./Smith_MLE_Speed,'b-x')
hold on
plot(ImgDimArr,GPU_LMFit_MLE_RD_Speed./Smith_MLE_Speed,'g-+')
axis tight
xlabel('Dimenstion D')
ylabel('GPU\_LMFit\_MLE_Speed/Smith\_MLE\_Speed')
legend('GPU\_LMFit\_RS\_MLE','GPU\_LMFit\_RD\_MLE');
